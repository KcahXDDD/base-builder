document.addEventListener('DOMContentLoaded', function() {
    ...
    // R key to rotate selected block
    document.addEventListener('keydown', function(e) {
        if (e.key.toLowerCase() === 'r' && selectedBlock) {
            let currentAngle = rotationAngles.get(selectedBlock) || 0;
            currentAngle += 45; // Rotate 45 degrees each press
            currentAngle = ((currentAngle % 360) + 360) % 360;
    ...
    // DELETE or BACKSPACE key to remove selected block
    document.addEventListener('keydown', function(e) {
        if ((e.key === 'Delete' || e.key === 'Backspace') && selectedBlock) {
            const structureType = selectedBlock.dataset.structure;
            selectedBlock.remove();
            selectedBlock = null;
            
            // Decrement the count for this structure type
            if (placedStructuresCount[structureType] !== undefined) {
                placedStructuresCount[structureType]--;
            }
            updateAllCounters();
        }
